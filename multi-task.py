...
from utils import SwinInferer, MultiTaskDataset, response_accuracy
from trainer import Trainer
from optimizers import LinearWarmupCosineAnnealingLR
from models.siamswinunetr import SiamSwinUNETR
...
def main(args) -> None:
    ...
    if args.pretrained_path:
        ...
        if args.use_ssl_pretrained:
            ...
            model.swin_unetr.load_from(weights)
        else:
            # Use saved weight dict (SwinUNETR weights)
    ...
    # Transforms
    trainer.post_seg_pred = lambda x: torch.argmax(x, dim=1, keepdim=True)

    # Losses
    trainer.seg_loss_func = DiceCELoss(include_background=True, to_onehot_y=True, softmax=True)
    trainer.response_loss_func = BCEWithLogitsLoss()

    # Metrics
    trainer.seg_acc_func = DiceMetric(
        include_background=False,
        get_not_nans=True,
        num_classes=args.out_channels,
        reduction="mean_batch")
    trainer.response_acc_func = response_accuracy
